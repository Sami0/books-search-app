...
/**
 * Create a new list with the give name, then list bundles.
 */
...
const getBooks = bookId => fetchJSON(`/api/book/${encodeURIComponent(bookId)}`);
...
const showView = async () => {
  ...
  switch (view) {
    ...
    case '#view-bundle':
      ...
      break;
    case '#view-book':
      const book = await getBooks(params[0]);
      await viewBook(book);
      break;
    default:
      // Unrecognized view.
